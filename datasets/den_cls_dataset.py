...
from utils.misc import random_crop, get_padding, my_fft, decoder_image
...
class DenClsDataset(DensityMapDataset):
    def collate(batch):
        ...
        H_img1 = torch.stack(transposed_batch[5], 0)/255
        H_img2 = torch.stack(transposed_batch[6], 0)/255
        L_img1 = torch.stack(transposed_batch[7], 0)/255
        L_img2 = torch.stack(transposed_batch[8], 0)/255
        ...
        return images1, images2,(H_img1,H_img2),(L_img1,L_img2), (points, dmaps, bmaps),
    ...
    def __getitem__(self, index):
        ...
        if self.method == 'train':
            ...
            img1, img2, gt, dmap,H_img1,H_img2,L_img1,L_img2 = self._train_transform(img, gt, dmap)
            ...
            return img1, img2, gt, dmap, bmap,H_img1,H_img2,L_img1,L_img2
    ...
    def _train_transform(self, img, gt, dmap):
        ...
        img_L_1,img_H_temp1, _ = my_fft(img1, 30)
        img_L_2,img_H_temp2, _ = my_fft(img2, 30)
        H_img1 = torch.from_numpy(np.array(img_H_temp1, dtype=np.float32))
        H_img2 = torch.from_numpy(np.array(img_H_temp2, dtype=np.float32))
        img_L_1 = torch.from_numpy(np.array(img_L_1, dtype=np.float32))
        img_L_2 = torch.from_numpy(np.array(img_L_2, dtype=np.float32))

        gt = torch.from_numpy(gt.copy()).float()
        dmap = dmap.float()

        return img1, img2, gt, dmap,H_img1,H_img2,img_L_1,img_L_2
